...
export default function EditQuestion() {
  ...
  const API_URL = `http://localhost:5000/api/questions/${id}`;
  ...
  const fetchData = async () => {
    try {
      ...
      const cRes = await fetch(CHAPTERS_URL);
      if (!cRes.ok) throw new Error("Failed to load chapters");
      ...
      setFormData({
        ...
        audio: null,
        audioPreview: qData.audio || "",
        chapterId: qData.chapterId?.toString() || "",
        testType: qData.testType || "FREE",
        status: qData.status === true,
  ...
  if (fetching) {
    return (
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "100vh", fontFamily: "system-ui", background: "#f0f9ff" }}>
